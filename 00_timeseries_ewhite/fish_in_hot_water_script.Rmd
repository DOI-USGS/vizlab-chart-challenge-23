...
# make long format
fish_data_ts_long <- gather(fish_data_ts, period, value, Onset:Cessation)
fish_data_ts_long$period <- factor(fish_data_ts_long$period, levels = c("Onset", "Cessation"))
```
...
ggplot(data = fish_data_ts_long[fish_data_ts_long$year > 1950, ], aes(x = year, y = value)) +
  ...
  facet_wrap(~condition+period, ncol =2, scales = "free") +
...
fish_data_ts |>
  ...
  mutate(AA = 
           sapply(Date, 
                  function(d) 
                    sum(between(as.numeric(difftime(d, Date[Date < d], units = "days")), 0, 31))
                  )
         )
# for each species, and each condition rank and get a cumulative probability of spawning before every date
ts <- fish_data_ts[fish_data_ts$condition == "rcp_26" & fish_data_ts$species == "American Shad", ]

ts_agg <- data.frame(day = (as.Date(1:365, origin = ("2015-01-01"))-1), 
                     count = NA)

aggregate(ts["Onset"], by = ts["Onset"], count)
