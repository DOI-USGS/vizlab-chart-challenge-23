...
theme_usgs <- function(legend.position = "right"){
  theme(
    plot.title = element_text(vjust = 3, size = 14, face = "bold", family="serif"),
    plot.subtitle = element_text(vjust = 3, size = 12,family="serif"),
...
ggplot(data = fish_data_long) + 
  ...
  geom_segment(data = fish_data_long_2, aes(x = min_onset, xend = max_end, y = y, yend = y), col = "#faf6fe", linewidth = 9.5, show.legend = FALSE) +
  geom_segment(aes(x = origin_date, xend = end_date, y = y, yend = y, col = period, group = period), linewidth = 7.5, alpha = 0.7, show.legend = FALSE) +
  ...
  labs(x = "", 
       ...
       subtitle = "The Striped Bass and American Shad of the Hudson River Estuary are projected to spawn earlier (~15 days) and have a shorter window (~4 days) to do so with climate change."
  ...
  theme_bw()+
  theme(plot.title = element_text(vjust = 3, size = 14, face = "bold", family="serif"),
        plot.subtitle = element_text(vjust = 3, size = 12,family="serif"), 
