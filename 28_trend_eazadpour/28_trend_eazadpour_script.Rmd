...
annual_forest_area_raw <- readr::read_csv(file_in_path_continuous) |> 
  ...
  forest_avg_diff <- forest_avg_join |> 
    mutate(forest_diff = (area - state_area_avg) / state_area_avg * 100)
```
...
(main_plot <- ggplot(my_grid_dropWI, aes(x = year, y = forest_diff)) +
  ...
  geom_segment(data = my_grid_dropWI |> filter(area_change %in% c("POSITIVE")), 
    ...
    geom_point(data = my_grid_dropWI |> filter(area_change %in% c("NEGATIVE")),
        ...
        scale_y_continuous(limits = c(-20, 20), breaks = c(-20,0,20), labels = paste(c("-20%", "0%", "20%"))) +
  ...
  labs(x = NULL, y= NULL) +
    ...
    theme(strip.background = element_blank(),
          ...
          plot.margin = margin(50, 40, 10, 40, "pt"),
...
(legend_plot <- ggplot(my_grid_conus |> filter(name %in% c('Wisconsin')), aes(x = year, y = forest_diff)) +
  ...
  theme(panel.grid.minor.y = element_blank(),
        ...
        text = element_text(family = font_legend, size = 10, color = "black"),
...
custom_plt <- ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_plot(legend_plot, .445, .83, .115, .15) +
  # explainer text
  ...
  draw_label(expression(paste("Forest area"~(km^2))),
    ...
    draw_label("difference from",
             ...
             color = font_color,
             size = 26) +
      draw_label("35-year mean (1985-2020) across",
             x = 0.035,
             y = 0.92,
             hjust = 0,
             vjust = 1,
...
2) North Dakota saw an increase of ~18% forest area in 2020 compared to the state's 35 year mean
