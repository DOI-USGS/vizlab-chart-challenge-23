...
test2021 <- full_record |> filter(year(dt) == 2019)
...
(main_plot <- ggplot(hyst_example, aes(x = airT, y = waterT)) +
    ...
    geom_point(color = font_color, size = inset_point_size, 
    ...
    ylim(c(47, 56))+
    xlim(c(44, 70))+
    geom_curve(aes(x = 46, xend = 55,
                     y = 48, yend = 49.2),
                 arrow = arrow(length = unit(0.03, "npc")),
                 color = font_color,
               angle = 150, curvature = 0.4, color = font_color)+
    ...
    geom_text(label = "day", y = 54.5, x = 45.5, hjust = -0,
             color = pop_color)+
    geom_text(label = "6 a.m.", y = 48.8, x = 45.5, hjust = -0,
             color = font_color)+
...
```{r facet}
(faceted_plot <- ggplot(whole_month, aes(x = airT, y = waterT)) +
    geom_point(color = pop_color, size = point_size)+
    geom_point(color = "white", size = inset_point_size, 
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_plot(main_plot,
            ...
            width = 0.85) +
  # explainer text
  draw_label("April 27, 2019 data from Paine Run, Shenandoah National Park.\nChart by Althea A. Archer. Data from https://doi.org/10.5066/P97ULKSM",
  ...
  draw_label("Hysteresis",
             ...
             size = 35) +
  # Title
  draw_label("A phenomenon in which the value of a physical property\nlags behind changes in the effect causing it.",
             x = 0.04,
             y = 0.9,
             hjust = 0,
             vjust = 1,
             ...
             color = font_color,
             size = 20) +
  draw_label("For example, here stream\ntemperature takes longer\nto warm during the day\nthan air temperature.",
             x = 0.45,
             y = 0.3,
             hjust = 0,
             vjust = 1,
