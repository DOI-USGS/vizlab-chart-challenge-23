...
theme_usgsmod <- function(legend.position = "right"){
  theme(
    ...
    legend.position = legend.position,
    legend.key = element_blank(),
    legend.title = element_text(size = 9, family="serif", margin = margin(5, 10, 10, 5, "pt")),
    legend.text = element_text(size = 9, family="serif"),
    axis.title.x = element_text(size = 9, family="serif", hjust = 0.047, margin = margin(0, 0, 130, 0, "pt")), # to have room to plot timeline
    axis.title.y = element_text(vjust = 1, angle = 0, size = 9, family="serif", margin = margin(0, -50, 0, 10, "pt")),
    axis.text.x = element_text(size = 9, vjust = 0, colour = "grey50",  
                               family="serif", margin=margin(10,5,20,5,"pt")),
    axis.text.y = element_text(size = 9, hjust = 0, colour = "grey50", 
                               family="serif", margin=margin(5,10,10,5,"pt")),
