...
ggsave("out/11_circular_ewhite_base1.png", width = 16, height = 9, units = "in", dpi = 1200)
...
group_data <- tibble(
  ...
  mutate(group_num = 8:1) # don't know why but interactions in ggplot are backwards, so, go decsending 

p_data <- full_join(layer_data(p), group_data, by = join_by(group == group_num))
  

ggplot(data = p_data, aes(x = x, y = y)) +
