...
# Main plot
main_plot <- ggplot(df_tidy, aes(x = year, y = amount*.data[[vol]])) + geom_col(aes(fill = use)) + xlab("Year") + ylab("Volume of Freshwater Withdrawal, billions of cubic meters") + ggtitle("Annual Freshwater Withdrawals in the United States (1990-2019)", subtitle="World Bank World Development Indicators Data") + labs(fill="EXPLANATION") + theme(panel.background = element_blank(), plot.background = element_rect(fill = background_color), text = element_text(colour = "white", size=14), legend.background = element_blank(), axis.text = element_text(colour = "white", size = 18), axis.ticks = element_line(colour = "white"), plot.title = element_text(size = 30), plot.subtitle = element_text(size = 22), axis.title = element_text(size = 18), legend.title = element_text(size=22), legend.text = element_text(size=18), plot.caption=element_text(size=14, colour="white", hjust=1)) + scale_fill_manual(values = c("#ff71ce", "#01cdfe", "#05ffa1")) + labs(caption="World Bank World Development Indicators Data, The World Bank Group, 2023.\nData acquisition and plotting done in the R programming language using the WDI, ggplot2, and tidyverse packages.") + gganimate::transition_time(year) + gganimate::shadow_mark() + gganimate::enter_grow() + gganimate::enter_fade()
...
gif_w_logo <- magick::image_composite(magick_gif, image_scale(usgs_logo, "150"), offset = "+10+840")
