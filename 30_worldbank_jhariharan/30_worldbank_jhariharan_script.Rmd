...
# Main plot
(main_plot <- ggplot(df_tidy, aes(x = year, y = amount*.data[[vol]])) + geom_col(aes(fill = use)) + xlab("Year") + ylab("Volume of Freshwater Withdrawal, billions of cubic meters") + ggtitle("Annual Freshwater Withdrawals in the United States (1990-2019)", subtitle="World Bank World Development Indicators Data") + labs(fill="EXPLANATION") + theme(panel.background = element_blank(), plot.background = element_blank(), text = element_text(colour = "white", size=14), legend.background = element_blank(), axis.text = element_text(colour = "white", size = 18), axis.ticks = element_line(colour = "white"), plot.title = element_text(size = 30), plot.subtitle = element_text(size = 24), axis.title = element_text(size = 22), legend.title = element_text(size=22), legend.text = element_text(size=18)) + scale_fill_manual(values = c("#ff71ce", "#01cdfe", "#05ffa1"))
 )
# (main_plot <- ggplot(df_tidy, aes(x=year, y=amount*.data[[vol]], fill=use)) + geom_bar(stat="identity", colour="white") + xlim(1989, 2020) + xlab("Year") + ylab("Volume of Freshwater Withdrawal, billions of cubic meters") + ggtitle("Annual Freshwater Withdrawals in the United States (1990-2019)", subtitle="World Bank World Development Indicators Data") + labs(fill="EXPLANATION") + theme(panel.background = element_blank(), plot.background = element_blank(), text = element_text(colour = "white", size=14), legend.background = element_blank(), axis.text = element_text(colour = "white", size = 18), axis.ticks = element_line(colour = "white"), plot.title = element_text(size = 30), plot.subtitle = element_text(size = 24), axis.title = element_text(size = 22), legend.title = element_text(size=22), legend.text = element_text(size=18))
#  )
...
anim_bar <- ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
       xlim = c(0,1)) +
  # a background
  draw_grob(canvas,
            x = 0, y = 1,
            ...
            hjust = 0, vjust = 1) +
  # the main plot
  draw_plot(main_plot + gganimate::transition_time(year) + gganimate::shadow_mark() + gganimate::enter_grow() + gganimate::enter_fade(),
            x = 0.01,
            y = 0.2,
            height = 0.7,
            width = 0.98) +
  # explainer text
  draw_label("World Bank World Development Indicators Data, The World Bank Group, 2023.\nData acquisition and plotting done in the R programming language using the WDI, ggplot2, and tidyverse packages.",
...
# animation
gganimate::animate(anim_bar, fps = 20)
