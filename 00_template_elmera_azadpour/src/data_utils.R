...
#' @param remove_missing_values, TRUE/FALSE arguement where TRUE = remove no data rows in date, value, comm, rmk. FALSE = retain NAs
...
filter_wells_wq <-  function(wq_tidy_data, wells_data, data_dic, filter_parm, remove_missing_values){
  
  filter_wq <-  wq_tidy_data |>
    # filter for constituents of interest 
    filter(parameter %in% c(filter_parm)) 
  
  # apply if statement removing missing values   
  if(remove_missing_values == TRUE){
    filter_wq <-  filter_wq |> 
      # filter out no data rows (when date/value/comm/rmk = NA)
      filter(!if_all(c(date, value, comm, rmk), is.na)) 
    
  } else if(remove_missing_values == FALSE){
  }
...
#' Summarizing variables of interest of filtered wq data (`p2_filter_decadal_gw`)
#'
#' @param wq_filt_data RDS of filtered Water_Quality_V6 data  
#' @return A summary RDS of count, mean, max, min of well_depth, date, and value, grouped by parameter and su_code
summarize_wq <-  function(wq_filt_data){
  
  # get date in class Date format 
  wq_filt_data$date <- lubridate::mdy(wq_filt_data$date)
  
  summarize_wq_filt <- wq_filt_data  |> 
    select(parameter, su_code, staid, well_depth, date, value, comm, rmk) |> 
    group_by(parameter, su_code) |> 
    summarise(count = n(),
              #well depth
              mean_depth = mean(well_depth),
              max_depth = max(well_depth),
              min_depth = min(well_depth),
