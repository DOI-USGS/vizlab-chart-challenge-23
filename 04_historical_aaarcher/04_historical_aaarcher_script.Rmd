...
library(scales)
library(zoo)
library(ggarrow) #devtools::install_github("teunbrand/ggarrow")
...
# Calculate average and ranges across all sites by generation
climate_metrics_CONUS_temp <- climate_metrics_generation_site_base |> 
  group_by(generation) |> 
...
(main_plot <- ggplot(data = plotting_data_CONUS, 
  ...
  geom_line(aes(y = CONUSmean_minflow_7d, 
                color = change_in_minflow, group = generation),
            linewidth = 2) +
    scale_color_scico(palette = "lapaz", direction = -1, begin = 0, end = 0.8) +
  ...
  theme(legend.position = "none",
    ...
    #scale_color_manual(values = c("blue","#999999","red")) #order: above, baseline, below
 )
# Chart settings
Ymargin <-  0.04
Xmargin <- 0.03
...
draw_CASC_labels <- map(1:length(CASC_names), function(x){
  ...
  draw_label(sprintf("%s%%", round(
    ...
    size = 25,
    color = ifelse(CASC_names[x] %in% c("Southwest", "Southeast"), "#ef8a62", "#005A9C"))
})
...
(map_plot <- ggplot(data = mapping_data) +
    ...
    theme_void()+
    scale_fill_scico(palette = "lapaz", direction = -1, begin = 0, end = 0.8) +
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_label("Minimum Annual Streamflow\nduring Gen Z (1997-2020)\nwas\n\n\n\non average than during the\nSilent Generation (1920-1945)",
             ...
             color = font_color,
             size = 25) +
  draw_label(sprintf("%s%% higher", CONUS_statistic), x = Xmargin + 0.15, y = 0.66,
            hjust = 0.5, vjust = 0, size = 52, fontfamily = font_legend, color = "#005A9C")+
