...
baselines <- climate_metrics_generation_site |> 
  ...
  select(baseline_generation, baseline_minflow_7d, StaID)

# Merge baselines and join metadata and calculate percent change by station
...
# convert id to CASC for mapping just regions
...
draw_CASC_plots <- map(1:length(CASC_names), function(x){
  plot_temp <- plotting_data_CASC |>
    ...
    geom_line(data = plotting_data_CASC |> 
              ...
              linewidth = 2) +
    scale_color_viridis_c(limits = c(0,147), direction = -1, end = 0.7) +
        geom_line(data = plotting_data_CASC |> 
...
(map_plot <- ggplot(data = mapping_data) +
    ...
    theme_void()+
    scale_fill_viridis_c(limits = c(0,147), direction = -1, end = 0.7) +
