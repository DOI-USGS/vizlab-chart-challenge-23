...
draw_CASC_plots <- map(1:length(CASC_names), function(x){
  ...
  if(CASC_names[x] %in% c("California", "Southwest", "Southeast", "South Central")){
    Y_position <- Ybase
  } else {
    Y_position <- Ybase + Yjitter
  }
  
  if(CASC_names[x] %in% c("Northwest", "California")){
    X_position <- Xbase
  } else if(CASC_names[x] %in% c("North Central", "Southwest")){
    X_position <- Xbase + Xjitter
  } else if(CASC_names[x] %in% c("Midwest", "South Central")){
...
(map_plot <- ggplot(data = mapping_data) +
  ...
  scico::scale_fill_scico(palette = "vik", direction = -1,
                          limits = c(-1, 1) * max(abs(mapping_data$change_in_minflow)))
)
