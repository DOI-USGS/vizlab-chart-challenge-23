...
t1921_data <- purrr::map(files_1921[1], 
  ...
  select(StaID, jd, dt, year, value, thresh_2_jd_07d_wndw) |>
  # and rename based on timeframe
  rename(thresh_2_1921 = thresh_2_jd_07d_wndw) 
...
climate_metrics_site <- t1921_data |> 
  ...
  group_by(StaID) |>
  # calculate minimum and max streamflow by site (returns Inf for empty sets)
  mutate(min_flow_7d = zoo::rollapply(value, width = 7, function(x){min(x, na.rm = TRUE)}, fill = NA),
         max_flow_3d =  zoo::rollapply(value, width = 3, function(x){max(x, na.rm = TRUE)}, fill = NA)) |>
  # convert infinities to missing values
  mutate(min_flow_7d = case_when(min_flow_7d %in% c(Inf, -Inf) ~ NA,
                                 TRUE ~ min_flow_7d),
         max_flow_3d = case_when(max_flow_3d %in% c(Inf, -Inf) ~ NA,
                                 TRUE ~ max_flow_3d))

# Extract MIN/MAX values by station and year
climate_metrics_year_site <- climate_metrics_site |>
  group_by(year, StaID) |> 
  summarise(minflow_7d = min(min_flow_7d, na.rm = TRUE),
            maxflow_3d = max(max_flow_3d, na.rm = TRUE)) |>
  # convert infinities to missing values
  mutate(minflow_7d = case_when(minflow_7d %in% c(Inf, -Inf) ~ NA,
                                 TRUE ~ minflow_7d),
         maxflow_3d = case_when(maxflow_3d %in% c(Inf, -Inf) ~ NA,
                                 TRUE ~ maxflow_3d))
...
climate_metrics_generation_site_temp <- climate_metrics_year_site_join |>
  ...
  summarise(minflow_7d = mean(minflow_7d, na.rm = TRUE),
            maxflow_3d = mean(maxflow_3d, na.rm = TRUE)) 
...
# Extract baseline values to get % changes by station
baselines <- climate_metrics_generation_site |> 
  rename(baseline_generation = generation,
         baseline_minflow_7d = minflow_7d,
         baseline_maxflow_3d = maxflow_3d) |>
  filter(baseline_generation == "0 silent generation") |>
  select(baseline_generation, baseline_minflow_7d, baseline_maxflow_3d, StaID)

# Merge baselines and join metadata and calculate percent changes
climate_metrics_generation_site_base <- climate_metrics_generation_site |>
  left_join(baselines, by = "StaID") |>
  mutate(change_minflow_7d = ((minflow_7d - baseline_minflow_7d)/baseline_minflow_7d)*100,
         change_maxflow_3d = ((maxflow_3d - baseline_maxflow_3d)/baseline_maxflow_3d)*100) |>
  # remove infinities
  mutate(change_minflow_7d = case_when(change_minflow_7d %in% c(Inf, -Inf) ~ NA,
                                       TRUE ~ change_minflow_7d),
         change_maxflow_3d = case_when(change_maxflow_3d %in% c(Inf, -Inf) ~ NA,
                                       TRUE ~ change_maxflow_3d))

# Calculate average and ranges across all sites
climate_metrics_CONUS <- climate_metrics_generation_site_base |> 
  group_by(generation) |> 
  summarise(mean_change_minflow_7d = mean(change_minflow_7d, na.rm = TRUE),
            median_change_minflow_7d = median(change_minflow_7d, na.rm = TRUE),
            mean_change_maxflow_3d = mean(change_maxflow_3d, na.rm = TRUE),
            median_change_maxflow_3d = median(change_maxflow_3d, na.rm = TRUE),
            CONUSmean_minflow_7d = mean(minflow_7d, na.rm = TRUE),
            CONUSmedian_minflow_7d = median(minflow_7d, na.rm = TRUE),
            CONUSmean_maxflow_3d = mean(maxflow_3d, na.rm = TRUE),
            CONUSmedian_maxflow_3d = median(maxflow_3d, na.rm = TRUE),
            n = n(),
            baseline_flow = 
              climate_metrics_CONUS$CONUSmean_minflow_7d[
                climate_metrics_CONUS$generation == "0 silent generation"],
...
climate_metrics_CASC_temp <- climate_metrics_generation_site_base |> 
  ...
  summarise(mean_change_minflow_7d = mean(change_minflow_7d, na.rm = TRUE),
            ...
            mean_change_maxflow_3d = mean(change_maxflow_3d, na.rm = TRUE),
            median_change_maxflow_3d = median(change_maxflow_3d, na.rm = TRUE),
            CASCmean_minflow_7d = mean(minflow_7d, na.rm = TRUE),
            CASCmedian_minflow_7d = median(minflow_7d, na.rm = TRUE),
            CASCmean_maxflow_3d = mean(maxflow_3d, na.rm = TRUE),
            CASCmedian_maxflow_3d = median(maxflow_3d, na.rm = TRUE),
            n = n())
...
plotting_data_CONUS <- climate_metrics_year_site |> 
  ...
  summarise(CONUS_annual_minflow_7d = mean(minflow_7d, na.rm = TRUE),
            CONUS_annual_maxflow_3d = mean(maxflow_3d, na.rm = TRUE)) |>
  mutate(generation = case_when(year %in% 1920:1945 ~ "0 silent generation",
                                year %in% 1946:1964 ~ "1 baby boomers",
                                year %in% 1965:1980 ~ "2 gen x",
                                year %in% 1981:1996 ~ "3 millenials",
...
plotting_data_CASC <- climate_metrics_year_site |>
  ...
  summarise(CASC_annual_minflow_7d = mean(minflow_7d, na.rm = TRUE),
            CASC_annual_maxflow_3d = mean(maxflow_3d, na.rm = TRUE)) |>
  mutate(generation = case_when(year %in% 1920:1945 ~ "0 silent generation",
                                year %in% 1946:1964 ~ "1 baby boomers",
                                year %in% 1965:1980 ~ "2 gen x",
                                year %in% 1981:1996 ~ "3 millenials",
...
mapping_data <- states_CASC |> left_join(climate_metrics_CASC |> filter(generation == "4 gen z"),
                                         by = "CASC")
...
(main_plot <- ggplot(data = plotting_data_CONUS, 
  ...
  geom_line(aes(y = CONUSmean_minflow_7d, color = is_change_negative, group = generation),
            linewidth = 2) +
  theme(legend.position = "none",
    ...
    scale_color_manual(values = c("blue","black","red")) #order: above, baseline, below
 )
# Chart settings
Ymargin <-  0.04
Xmargin <- 0.03
...
draw_CASC_plots <- map(1:length(CASC_names), function(x){
  plot_temp <- plotting_data_CASC |>
    ...
    geom_line(aes(y = CASCmean_minflow_7d, group = generation, color = is_change_negative),
              linewidth = 2) +
    ...
    scale_color_manual(values = c("blue","black","red"),
                       breaks = c("no", "no change", "yes")) #order: above, baseline, below
...
draw_CASC_labels <- map(1:length(CASC_names), function(x){
  ...
  if(CASC_names[x] %in% c("Southwest", "Southeast", "South Central")){
    Y_position <- Ybase + Yjitter - 0.1
  } else if(CASC_names[x] %in% c("California", "Northeast")){
    Y_position <- Ybase + 2*Yjitter - 0.1
  } else {
    Y_position <- Ybase + 3*Yjitter - 0.1
  }
  
  if(CASC_names[x] %in% c("Northwest", "California", "Southwest")){
    X_position <- Xbase - (2*Xjitter) -0.1
  } else if(CASC_names[x] %in% c("North Central", "South Central")){
    X_position <- Xbase - Xjitter - 0.1
  } else {
    X_position <- Xbase - 0.1
  } 
  ...
  draw_label(sprintf("%s%%", round(
    ...
    size = 25,
    color = ifelse(CASC_names[x] %in% c("Southwest", "Southeast"), "red", "blue"))
})


(map_plot <- ggplot(data = mapping_data |> filter(change_in_minflow > 0)) +
    geom_sf(aes(fill = change_in_minflow), color = "white") +
    theme_void()+
    scale_fill_gradient2(low = "#68BBE3", mid = "#68BBE3", high = "#000075")+
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_text("Gen X", 
            x = Xmargin + 0.175, y = 0.49, size = 12) +
  ...
  draw_CASC_labels +
  # explainer text
  draw_label("Minimum Streamflow values are based on annual 7-day low flows from 331 gages for which there are 100 years of data.\nData from doi.org/10.5066/P92FAASD. Chart by A. Archer.",
             ...
             color = "#999999")+
  # Title
  draw_label("These aren't your parents' streamflows!",
             x = Xmargin,
             y = 1-Ymargin,
             hjust = 0,
             vjust = 1,
             ...
             size = 45) +
    # Statistics
  draw_label("Minimum Annual Streamflow\nhas increased across the U.S. by\n\n\n\nsince the\nSilent Generation (1920 - 1945)",
             x = Xmargin + 0.15,
             y = 0.85,
             hjust = 0.5,
             vjust = 1,
             ...
             color = font_color,
             size = 25) +
  draw_label(sprintf("%s%%", CONUS_statistic), x = Xmargin + 0.15, y = 0.69,
            hjust = 0.5, vjust = 0, size = 52, fontfamily = font_legend, color = "blue")+
  draw_label("These changes in Annual Minimum Streamflow vary by region",
             x = Xmargin + 0.35,
             y = 0.85,
             hjust = 0,
             vjust = 1,
