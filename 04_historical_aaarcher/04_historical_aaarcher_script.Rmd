...
t1921_data <- purrr::map(files_1921[1], 
  ...
  select(StaID, jd, thresh_20_jd_07d_wndw) |>
  # and rename based on timeframe
  rename(thresh_1921 = thresh_20_jd_07d_wndw) 



files_1981 <- sprintf("in/Streamflow_percentiles_national_1981/%s.csv", focal_sites)
t1981_data <- purrr::map(files_1981[1], 
  ...
  filter(year == 1983) |>
  # only keep fields we need
  select(StaID, jd, dt, thresh_20_jd_07d_wndw) |>
  # and rename based on timeframe
  rename(thresh_1981 = thresh_20_jd_07d_wndw) 

# Merge data together (left join)
thresh_data <- t1921_data |> left_join(t1981_data) |> 
...
random_sites <- c("05454500", "10337500", "02223000")#sample(focal_sites, size = 25)
thresh_random <- all_data |>
  filter(StaID %in% random_sites) |>
  mutate(increase_ymin = case_when(thresh_1981 > thresh_1921 ~ pmin(thresh_1981, thresh_1921)),
         increase_ymax = case_when(thresh_1981 > thresh_1921 ~ pmax(thresh_1981, thresh_1921)),
         decrease_ymin = case_when(thresh_1981 < thresh_1921 ~ pmin(thresh_1981, thresh_1921)),
         decrease_ymax = case_when(thresh_1981 < thresh_1921 ~ pmax(thresh_1981, thresh_1921)))

# https://github.com/tidyverse/ggplot2/issues/2815
cp <- coord_polar(theta = "x")
cp$is_free <- function() TRUE
ggplot(data = thresh_random, aes(y = thresh_1921, x = jd, group = StaID)) + 
  geom_line(color = "red", alpha = 0.2) + 
  geom_line(aes(y = thresh_1981, group = StaID), color = "mediumblue", alpha = 0.2)+
  geom_ribbon(aes(ymin = increase_ymin,
                  ymax = increase_ymax),
              fill = "blue", alpha = 0.5)+
  geom_ribbon(aes(ymin = decrease_ymin,
                  ymax = decrease_ymax),
              fill = "red", alpha = 0.5)+
  #cp +
  geofacet::facet_geo(~ STATE, scales = "free_y") + 
  theme(
...
ggplot(data = thresh_random, aes(y = thresh_1921, x = dt, group = StaID)) + 
  geom_line(color = "red", alpha = 0.2) + 
  geom_line(aes(y = thresh_1981, group = StaID), color = "blue", alpha = 0.2)+
  geom_ribbon(aes(ymin = increase_ymin,
                  ymax = increase_ymax),
              fill = "blue", alpha = 0.2)+
  geom_ribbon(aes(ymin = decrease_ymin,
              ...
              fill = "red", alpha = 0.3)+
  #cp +
  facet_wrap(~ StaID, scales = "free_y", nrow = 3) + 
  theme(
    strip.background = element_blank(),
    #strip.text.x = element_blank(),
    axis.text = element_blank()
...
california_example_all <- readr::read_csv("in/Streamflow_percentiles_national_1921/10337500.csv")  |>
  mutate(decade = floor(year/10)*10)
california_example21 <- readr::read_csv("in/Streamflow_percentiles_national_1921/10337500.csv") |>
  filter(year == 1985)
california_example81 <- readr::read_csv("in/Streamflow_percentiles_national_1981/10337500.csv") |>
  filter(year == 1985)
ggplot(data = california_example_all |> filter(year == 1990 | year == 1932), 
       aes(x = jd, y = value, group = year)) + 
  geom_line(aes(color = year), alpha = 0.5) +
  geom_line(data = california_example21, aes(y = thresh_20_jd), color = "pink") +
  geom_line(data = california_example81, aes(y = thresh_20_jd), color = "maroon") +
  #coord_polar() +
  #ylim(c(0,200)) +
  #facet_wrap(~ decade) + 
  theme_void()
