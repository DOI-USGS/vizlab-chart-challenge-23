...
2. Put all R code in the chunks. They're organized for you to do set-up first (load libraries), load in any data (manually or with built in packages), process your data for plotting, create plot using ggplot, and finally compose the twitter image with cowplot.
...
4. To preview the final plot, use the `Knit` button above. That will create a preview of the final plot in the output `html` and will save the `png` in the `out/` folder. 
...
library(magick) # for coloring the logo to match composition
```
...
> Note: The hardest part of this process is getting text to be the optimum size for reading. The font sizes here will **not** be the same as in the final image after the cowplot step. Make sure to check the output `png` and the final plot in the `html` below for true font sizing. 
...
Here, use `cowplot` and `ggsave` to create the final viz for sharing out on Twitter. This template includes adding and coloring the USGS logo.
...
ggsave(filename = "out/exampleCC_Twitter.png", 
...
Tis the season to be snowy! Choropleth map of the contiguous U.S. with each stated colored by its percent snow cover for the year 2020. The snowiest states are Maine, Minnesota, Idaho, and Wyoming. The least snowy states include the southeastern U.S., such as Florida, Georgia, Alabama, Louisiana, Mississippi, and Texas.
