...
fish_data_uncertainty_long <- gather(fish_data_uncertainty, condition, confidence_interval, RCP_26_SE:RCP_85_SE)
...
ggplot() +
  ...
  geom_segment(data = fish_data_wide[fish_data_wide$period == "Historical", ], aes(x = onset-onset_se, xend = cessation + cessation_se, y = y_offset, yend = y_offset), alpha = 0.25, linewidth = 8 , show.legend = FALSE, col = "grey") + # for standard error end bits
  new_scale_colour() +
  ...
  geom_segment(data = fish_data_wide[fish_data_wide$period == "Future", ], aes(x = onset-onset_se, xend = cessation + cessation_se, y = y_offset, yend = y_offset), alpha = 0.25, linewidth = 8 , show.legend = FALSE, col = "#815aa5") + # for standard error end bits
...
## Tests and Checks
```{r checking_original_data_with_extra_data}
...
# never mind, we aren't going to recreate the author's analysis, going to take the paper values as they are
```
...
animate(animation, end_pause = 30, width = 16, height = 9, units = "in", res = 300, renderer = gifski_renderer())
anim_save("out/26_local-change_ewhite.gif")
```

```{r plotting_annual}
# or maybe just a static pic 
ggplot() +
  geom_segment(data = fish_data_ts_plot[fish_data_ts_plot$species=="American Shad", ], aes(x = Onset, xend = Cessation, y = year, yend = year, col = Onset), linewidth = 1, show.legend = FALSE) +
  scale_color_scico(palette = "tokyo", guide = guide_colorbar(order = 1)) +
  new_scale_colour() +
  geom_segment(data = fish_data_ts_plot[fish_data_ts_plot$species=="Striped Bass", ], aes(x = Onset, xend = Cessation, y = year, yend = year, col = Onset), linewidth = 1, show.legend = FALSE) +
  scale_color_scico(palette = "tokyo", guide = guide_colorbar(order = 2)) +
  scale_x_date(limits = c(as.Date("2015-01-01"), as.Date("2015-12-31")), date_breaks = "1 month", date_minor_breaks = "1 week", date_labels = "%B") +
  scale_y_continuous(limits = c(1900, 2120)) +
  coord_polar(theta = "x", direction = 1, start = -1.57*1.5) + # start is in radians, 90 Deg is Jan
  facet_wrap(~species) +
...
1) produced `out/25_local-change_ewhite_base.png` with ggplot

2) made markups in PowerPoint 

3) final plot is called `out/25_local-change_ewhite_final.png`
