...
```{r plotting2}
state_fips <- (unique(state_wu$state_cd))
state_plots <- purrr::map(state_fips, function(state_code) {
    # geom_area(data = filter(munged_state, state_cd == state_code), aes(year, total_watersource_mgd, fill = wu_category))
  geom_alluvium(data = filter(munged_state, state_cd == state_code), aes(year, total_watersource_mgd, fill = wu_category, alluvium = wu_category), decreasing = FALSE, alpha = 0.8)
})
...
make_carto_grid <- function() {
  us_state_grid1 %>% #us_state_without_DC_grid1
    ...
    filter(code != "DC") # remove DC (doesn't have counties)
}
