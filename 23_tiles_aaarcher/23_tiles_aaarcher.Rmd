...
gages_with_drought_decade <- droughts_100_join_CASC |> 
...
(map_plot <- ggplot(data = states_CASC) +
    geom_sf(aes(fill = CASC), color = "white") +
...
```{r plot_tests1}
(main_plot <- ggplot(data = droughts_long, 
...
# CASC Plots
CASC_names <- unique(droughts_100_join_CASC$CASC)
Xbase <- 1 - Xmargin
...
draw_CASC_plots <- map(1:length(CASC_names), function(x){
  temp_data <- droughts_long |> filter(CASC == CASC_names[x])
