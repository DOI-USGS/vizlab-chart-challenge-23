...
(map_plot <- ggplot(data = states_CASC) +
   ...
   # most severe called out
      geom_point(data = droughts_1000perRecord_100yr |> filter(severity == 626.90),
              aes(x = LNG_GAGE, y = LAT_GAGE),
              color = "black", size = 4)+
   geom_point(data = droughts_1000perRecord_100yr |> filter(severity == 626.90),
              aes(x = LNG_GAGE, y = LAT_GAGE),
              color = pop_color, size = 2.8)+
   theme_void()+
...
draw_decade_plots <- map(1:length(decades), function(x){
  ...
  plot_temp <- ggplot(data = states_CASC) +
   ...
   geom_point(data = droughts_100perDecade_100yr |> filter(decade == decades[x]),
              ...
              color = pop_color, size = 3)+
   theme_void()+
    theme(strip.text = element_text(size = support_size, color = font_color,
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_image(usgs_logo, 
             ...
             halign = 0, valign = 0)+
  # Most severe drought
  geom_curve(aes(x = Xbase - Xjitter*4 - 0.1, xend = Xbase - Xjitter*4 - 0.05,
                 y = Ybase + Yjitter*3 - 0.08, yend = Ybase + Yjitter*3 + 0.012),
             ...
             lineend = 'round',
             linewidth = 1.5)+
  draw_text("The most severe drought\non record was also the longest,\nlasting from March 10, 1924 to Feb. 12, 1926",
            y = 0.455,
            x = 0.132,
