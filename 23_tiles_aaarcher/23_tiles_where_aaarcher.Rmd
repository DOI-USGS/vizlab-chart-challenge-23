...
droughts_100perDecade_100yr <- droughts_2pct_100yr |>
  # Group by decade
  group_by(decade) |>
...
droughts_100perRecord_100yr <- droughts_2pct_100yr |>
  # and get top 100
  slice_max(order_by = severity, n = 100)|>
  left_join(metadata, by = "StaID")
...
(map_plot <- ggplot(data = states_CASC) +
   geom_sf(fill = "#e6e9f0", color = background_color, linewidth = 0.5) +
   geom_point(data = droughts_100perRecord_100yr |> filter(historical == "Historical"),
              aes(x = LNG_GAGE, y = LAT_GAGE, size = severity/80),
              color = "black")+
   geom_point(data = droughts_100perRecord_100yr |> filter(historical == "Historical"),
              aes(x = LNG_GAGE, y = LAT_GAGE, color = historical, size = severity/100),
              color = "grey")+
   geom_point(data = droughts_100perRecord_100yr |> filter(historical == "Recent"),
              aes(x = LNG_GAGE, y = LAT_GAGE, size = severity/80),
              color = "black")+
   geom_point(data = droughts_100perRecord_100yr |> filter(historical == "Recent"),
              aes(x = LNG_GAGE, y = LAT_GAGE, color = historical, size = severity/100),
...
draw_decade_plots <- map(1:length(decades), function(x){
  ...
  plot_temp <- ggplot(data = states_CASC) +
   ...
   geom_point(data = droughts_100perDecade_100yr |> filter(decade == decades[x]),
              ...
              color = "black")+
   geom_point(data = droughts_100perDecade_100yr |> filter(decade == decades[x]),
              aes(x = LNG_GAGE, y = LAT_GAGE, color = historical, size = severity/110),
              color = pop_color)+
   theme_void()+
    theme(legend.position = "none")
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  # Title
  draw_label("These are the 100 most severe exceptional droughts\nsince 1920 across all 100-years and by decade.",
             x = Xmargin,
             y = 1 - Ymargin - Ymargin ,
             hjust = 0,
             vjust = 1,
