...
(CASC_season_plot <- ggplot(data = droughts_long, 
   ...
   geom_histogram(aes(fill = sprintf("%ss", decade)), boundary = 0, bins = 36)+
   facet_wrap(~ CASC)+
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_image(usgs_logo, 
             ...
             halign = 0, valign = 0)+
  # Y-axis explainer
  geom_segment(aes(x = Xbase - Xjitter*3 - 0.2137, xend = Xbase - Xjitter*3 - 0.2137, 
                   y = Ybase + Yjitter + 0.125, yend = Ybase + Yjitter*2 + 0.065), 
  ...
  draw_text("Number of\ndroughts",
            x = Xmargin + 0.02,
            y = Ybase + Yjitter*2 + 0.12,
            color = pop_color, size = support_size, family = annotation_font,
            hjust = 0)+
  # Facts
  # X-axis explainer
    geom_curve(aes(xend = Xbase - Xjitter*3 - 0.09, 
                   x = Xbase - Xjitter*3 - 0.2137,
                   yend = Ybase + Yjitter + 0.14, 
                   y = Ybase + Yjitter*2 + 0.09),
