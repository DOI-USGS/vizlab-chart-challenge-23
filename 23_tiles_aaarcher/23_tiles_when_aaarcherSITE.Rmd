...
# Drought Properties - site - 1921-2020 dataset
drought21_properties_site_file <- "in/Drought_Properties_site_1921.csv"
if(!file.exists(drought21_properties_site_file)){ # if files don't exist, download
  sbtools::item_file_download(sb_id = "62f463c5d34eacf5397395d9",
                              names = "Drought_Properties_site.csv",
                              destinations =  "in/Drought_Properties_site_1921.csv",
                              overwrite_file = FALSE)
}
temp_1921 <- readr::read_csv(drought21_properties_site_file) 

# Drought Properties - site - 1951-2020 dataset
drought51_properties_site_file <- "in/Drought_Properties_site_1951.csv"
if(!file.exists(drought51_properties_site_file)){ # if files don't exist, download
  sbtools::item_file_download(sb_id = "627974ccd34e8d45aa6e3c81",
                              names = "Drought_Properties_site.csv",
                              destinations =  "in/Drought_Properties_site_1951.csv",
                              overwrite_file = FALSE)
}
temp_1951 <- readr::read_csv(drought51_properties_site_file) 

# Drought Properties - site - 1981-2020 dataset
drought81_properties_site_file <- "in/Drought_Properties_site_81.csv"
if(!file.exists(drought81_properties_site_file)){ # if files don't exist, download
  sbtools::item_file_download(sb_id = "62f475abd34eacf5397395e8",
                              names = "Drought_Properties_site.csv",
                              destinations =  "in/Drought_Properties_site_81.csv",
                              overwrite_file = FALSE)
}
temp_1981 <- readr::read_csv(drought81_properties_site_file) 
...
month_labels <- tibble(
  drought_day_fakeYr = as.Date(sprintf("1999-%s-01", 
                                       c("01", "02", "03", "04", "05", "06",
                                         "07", "08", "09", "10", "11", "12"))),
  y_CONUS = 6000,
...
(full_plot <- ggplot(data = droughts_long, 
   ...
   geom_text(data = month_labels, 
             ...
             font = supporting_font, fontface = "bold") +
   scale_x_date(breaks = "3 month", date_labels = "%b", 
                limits = as.Date(c("1999-01-01", "1999-12-31")),
                expand = c(-0.015,0))+
   labs(fill = "Decade")+
...
ggdraw(ylim = c(0,1), # 0-1 scale makes it easy to place viz items on canvas
  ...
  draw_plot(full_plot,
            ...
            vjust = 0.5)+
  # explainer text
  draw_label("Chart by Althea A Archer. Data based on streamgage records from 3,196 gages assessed from 1920 to 2020.\nSevere streamflow drought events were defined with 2% fixed thresholds. https://doi.org/10.5066/P92FAASD.",
  ...
  # Title
  draw_label("These are the 2000 most severe streamflow droughts at gages from 1920 to 2020 by time of year, decade, and region.",
             x = Xmargin,
             y = 1 - 3*Ymargin ,
             hjust = 0,
             vjust = 1,
  ...
  draw_image(usgs_logo, 
             ...
             halign = 0, valign = 0)+
  # Y-axis explainer
  geom_segment(aes(x = Xbase - Xjitter*3 - 0.225, xend = Xbase - Xjitter*3 - 0.225, 
                   y = Ybase + Yjitter + 0.1, yend = Ybase + Yjitter*2 + 0.02), 
  ...
  draw_text("Number of\ndroughts",
            x = Xmargin + 0.02,
            y = Ybase + Yjitter*2 + 0.02,
            color = pop_color, size = support_size, family = annotation_font,
            hjust = 0)+
  # Facts
  # X-axis explainer
    geom_curve(aes(xend = Xbase - Xjitter*3 - 0.105, 
                   x = Xbase - Xjitter*3 - 0.225,
                   yend = Ybase + Yjitter + 0.1, 
                   y = Ybase + Yjitter*2 + 0.04),
             ...
             linewidth = 1.5)+
  draw_text("Day of\n   year",
            x = Xbase - Xjitter*3 - 0.15,
            y = Ybase + Yjitter*2 + 0.00,
            color = pop_color, size = support_size, family = annotation_font,
            hjust = 0)
...
ggsave(filename = "out/20230423_timeseries-tiles_whenDrought_aaarcherSITE.png", 
