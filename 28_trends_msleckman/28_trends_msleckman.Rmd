...
colors_lt1 <- get_colors(img='in/R/lake-tahoe-1591339_1280.jpeg')
colors_lt2 <- get_colors(img='in/R/Lake-Tahoe-in-summer.png')
colors_lt3 <- get_colors(img='in/R/lake_tahoe_w_boat.pmg.jpg')
# create palette, default palette size is 10
lt_colours_vector_1 <- make_palette(colors_lt1)
lt_colours_vector_2 <- make_palette(colors_lt2)
lt_colours_vector_3 <- make_palette(colors_lt3)

all_colors <- c(lt_colours_vector_1, lt_colours_vector_2, lt_colours_vector_3)
show_col(all_colors)
...
lt_map <- ggplot()+
  ...
  ## lakes (incl. all lakes in basin)
   geom_sf(data = LT_lakes |>
           ...
           fill = all_colors[7], 
           color = all_colors[7],  ## probably easier to write out huc but currently dont have R studio updated to show colors
           alpha = 0.8)+
...
site_lst <- unique(LT_dv_data$site_no)
print(site_lst)

site_lst_ordered <- c("10336676", "10336660", "10336645", "10336610", "103366092", "10336780", "10336730", "10336715", "10336700", "10336698")
...
#        day_of_week %in% c('Mon','Wed','Fri')
  ...
  mutate(site_no = factor(site_no, site_lst_ordered)) |> 
  arrange(site_no) |> 
  group_by(site_no) 
...
LT_dv_data_viz |>
  ...
  site_no == '103366092') |>
  ggplot()+
  geom_line(aes(x = fake_date, y = Flow, color = as.factor(year)))+
  geom_line(aes(x = fake_date, y = MA, color = as.factor(year)), linetype = 'dotted')+
  theme_minimal()
...
draw_line_plots <- purrr::map(1:length(line_plot_lst), function(x){
                        ...
                        # for 5 and 10
                         ...
                         if(x %in% c(1,9)){
                           Y_position <- 0.72 # 0.72
                        } else if(x %in% c(2,8)){
                           Y_position <- 0.55  # 0.56
                        } else if(x %in% c(3,7)){
                           Y_position <- 0.35 # 0.4
                        } else if(x %in% c(4,6)){
                           Y_position <- 0.18 # 0.24
                        } else if(x == 5){
                           Y_position <- 0.06 
                        } else {
                           Y_position <- 0.85
